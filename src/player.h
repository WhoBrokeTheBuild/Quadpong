...
player_get_color(player_t * player)
{
    return object_get_color(&player->base);
}
static inline void
player_set_color(player_t * player, color_t color)
{
    object_set_color(&player->base, color);
}

static inline vec2f_t
player_get_vel(player_t * player)
{
    return object_get_vel(&player->base);
}
static inline void
player_set_vel(player_t * player, vec2f_t vel)
{
    object_set_vel(&player->base, vel);
}

static inline vec2f_t
player_get_pos(player_t * ply)
{
    return object_get_pos(&ply->base);
}
static inline void
player_set_pos(player_t * ply, vec2f_t pos)
{
    object_set_pos(&ply->base, pos);
}

static inline area_t
player_get_area(player_t * ply)
{
    return ply->_area;
}
static inline SDL_Rect
player_get_bounding_box(player_t * ply)
{
...
void network_player_init(network_player_t *   ply,
                         area_t               area,
                         SDL_Color            color,
                         struct sockaddr_in * addr,
                         socklen_t            addrlen);
