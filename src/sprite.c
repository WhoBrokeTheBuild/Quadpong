...
bool
sprite_load_file(sprite_t * spr, const char * filename)
{
    int w, h;
...
bool
sprite_create(sprite_t * spr, int w, int h, color_t color)
{
    uint8_t pixel[] = {255, 255, 255, 255};
    spr->_texture =
        SDL_CreateTexture(g_renderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STATIC, w, h);
    if (NULL == spr->_texture)
    {
        return false;
    }
...
void
sprite_set_align(sprite_t * spr, align_t align)
{
    spr->_align = align;
    _sprite_apply_align(spr);
}
...
void
sprite_set_pos(sprite_t * spr, vec2f_t pos)
{
    spr->_pos   = pos;
...
void
sprite_set_size(sprite_t * spr, vec2_t size)
{
    spr->_dst.w = size.x;
    spr->_dst.h = size.y;
    _sprite_apply_align(spr);
...
void
sprite_set_color(sprite_t * spr, color_t color)
{
    spr->_color = color;
    _sprite_apply_color(spr);
}
...
void
_sprite_apply_align(sprite_t * spr)
{
...
void
_sprite_apply_color(sprite_t * spr)
{
    if (NULL != spr->_texture)
    {
