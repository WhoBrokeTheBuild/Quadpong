...
static inline align_t sprite_get_align(sprite_t *spr)
{
    return spr->_align;
}
...
{
    return (vec2_t){spr->_dst.w, spr->_dst.h};
}
void sprite_set_size(sprite_t *spr, vec2_t size);

static inline color_t sprite_get_color(sprite_t *spr)
{
    return spr->_color;
}
void sprite_set_color(sprite_t *spr, color_t color);

static inline rect_t sprite_get_bounding_box(sprite_t *spr)
{
    return spr->_dst;
}
